...
協助一般使用者把居家／裝潢靈感整理成 **結構化文字需求**、**視覺 moodboard**，並可匯出 **PDF 設計摘要**，方便與室內設計師溝通。

畫面與持久化以瀏覽器為主；**圖片網址**會由 Next.js **API Route**（`/api/fetch-image`）在伺服端抓取並轉成內嵌圖，以便匯出 PDF 時不受跨網域阻擋。未接使用者帳號或雲端資料庫。
...
| **家具意向**       | 名稱、選填品牌與圖片；Moodboard／PDF 僅顯示家具圖                      |
| **Moodboard 預覽** | 同頁顯示設計摘要、色系示意、最多 12 張拼貼                              |
| **匯出 PDF**       | 將預覽區截圖輸出為 A4 PDF（供列印或傳給設計師）                         |

**目前刻意不包含：** AI 圖片分析、自動風格辨識、雲端帳號、線上分享連結（見下方「後續規劃」與 PRD）。
...
3. 上傳靈感圖或貼上圖片 URL（必要時填 **來源標記**）；可新增 **家具意向** 圖片。
