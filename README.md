...
目前為 **第一版（MVP）**：畫面與持久化以瀏覽器為主；**圖片網址**會由 Next.js **API Route**（`/api/fetch-image`）在伺服端抓取並轉成內嵌圖，以便匯出 PDF 時不受跨網域阻擋。未接使用者帳號或雲端資料庫。
...
| **靈感圖片**       | 本機上傳（多張、單張有大小上限），或貼 **http(s)** 網址（由伺服端代抓並存成與上傳相同的格式）；可標記來源 |
...
│   │   └── api/fetch-image   # 代理下載圖片網址 → data URL（供 PDF）
...
- 空間、文字、標籤與圖片引用皆存在瀏覽器 **`localStorage`**（鍵名：`interior-mood-translator-rooms`）。
- **貼上圖片網址**時，瀏覽器會呼叫本機 Next API **下載該圖**（僅轉傳為 base64 回前端），**應用程式不會**把圖片另存到自己的資料庫；但若部署在雲端主機，請留意該主機對外連線之資安與濫用（路由內已阻擋常見內網／localhost 網址以降低 SSRF 風險）。
- 網址圖下載成功後會以 **Data URL** 存入 localStorage，與本機上傳類似，**容量較大**，請勿一次塞過多張超大圖。
- 清除網站資料或換瀏覽器／裝置後，資料**不會**同步還原。
- **舊版曾存入的「純 https 網址」**若未經代抓轉換，匯出 PDF 仍可能失敗；請刪除該張後重新「加入網址圖片」。
...
- **新加入的網址圖**會先經 `/api/fetch-image` 轉成內嵌圖，一般可正常匯出。若遠端網站拒絕伺服端抓取（403、需要 Cookie 等），請改 **本機上傳** 該圖。
